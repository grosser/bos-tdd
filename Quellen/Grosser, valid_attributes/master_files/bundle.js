...
return this}
$.hotkeys.cache={}
$.hotkeys.special={'enter':13,'?':0,'/':223}
$.hotkeys.specialCodes=[]
$.each($.hotkeys.special,function(i,e){$.hotkeys.specialCodes.push(e)})})(jQuery)
...
if($.inArray(e.keyCode,$.hotkeys.specialCodes)>-1){var code=e.keyCode}else{var code=e.shiftKey?e.keyCode:(e.keyCode+32)}
var el=$.hotkeys.cache[code]
...
return jQuery.ajax({type:"DELETE",url:url,data:data,success:callback,dataType:type});}})(jQuery);(function($){$.fn.spamjax=function(callback,settings){if($.isFunction(settings)){var s=callback,callback=settings,settings=s}
...
$('#selected_author_text').remove()}};GitHub.Issues={active:null,repoURL:null,init:function(){var issue
GitHub.Issues.repoURL=window.location.pathname.match(/^\/[^\/]+\/[^\/]+/)[0]+'/issues/'
...
GitHub.Issues.adjustViewForTarget()}},adjustViewForComment:function(comment){var el=$('#comment_'+comment)
...
comment.find('form').hide()},isDraggable:function(){return!/sort=\w+/.test(location.search)},showIssue:function(issue){GitHub.Issues.active=issue
...
if(github_user!=null&&GitHub.Issues.collab){active.find('.label .remove').show()
...
$('#issues #loading').hide()},markAsRead:function(issue){var active=issue instanceof jQuery?issue:$('#issue_'+issue),id=active.attr('id').split('_')[1]
if(!active.hasClass('read')){active.addClass('read')
...
GitHub.Issues.incrementUnreadCount()},decrementUnreadCount:function(){var unread=parseInt($('#unread_count').text()),count=unread-1
if(count==0){$('#unread_count').parent().text('Open')}else{$('#unread_count').text(unread-1)}},incrementUnreadCount:function(){var unread=$('#unread_count')
if(unread.length==0){unread=' (<span id="unread_count">1</span>)'
$('#open_issues_link').append(unread)}else{unread.text(parseInt(unread.text())+1)}},changeOpenCount:function(delta){var tabText=$('#repo_menu li.active a').text()
var matches=tabText.match(/Issues \((\d+)\)/)
var newCount=parseInt(matches[1])+delta
$('#repo_menu li.active a').text('Issues ('+newCount+')')},decrementOpenCount:function(){this.changeOpenCount(-1)},incrementOpenCount:function(){this.changeOpenCount(1)},hideIssue:function(){this.hideEditCommentForms()
...
$('#issues #loading').hide()
GitHub.Issues.adjustViewForTarget()},openIssue:function(issue){var div=$('#issue_'+issue)
if(GitHub.Issues.active){issue=GitHub.Issues.active
GitHub.Issues.hideIssue()}
if(!div.is('.read')){GitHub.Issues.incrementUnreadCount()}
...
issue.click(GitHub.Issues.issueRowClickHandler)
...
return false},createIssue:function(){$('.create_issue:first').click()},createLabel:function(){$('.create_label').click()},backToInbox:function(){if(GitHub.Issues.active){$('.back_link').click()}else{window.location=$('#open_issues_link').attr('href')}},backToIssues:function(){$('.back_link').click()},markSelectedAsRead:function(){$('.selected').each(function(){GitHub.Issues.markAsRead($(this))})},markSelectedAsUnread:function(){$('.selected').each(function(){GitHub.Issues.markAsUnread($(this))})},closeSelected:function(){$('#issues #action_list').get(0).selectedIndex=1
...
$('#issue_'+issue).addClass('target')},targetNone:function(){GitHub.Issues.target.removeClass('target')},moveTargetDown:function(){if(GitHub.Issues.active){var nextIssue=$('#issue_'+GitHub.Issues.active).next()
if(nextIssue.size()){nextIssue.addClass('target')
GitHub.Issues.showTarget()}}else{$('#issue_list li').not(':last').filter('.target').removeClass('target').next().addClass('target')
...
GitHub.Issues.showTarget()}}else{$('#issue_list li').not(':first').filter('.target').removeClass('target').prev().addClass('target')
...
box.change()},get target(){return $('.target')},focusIssuesSearch:function(){$('#issues .search :text').focus()},showHotkeyHelp:function(){if(GitHub.Issues.keyboardShortcuts)return $.facebox(GitHub.Issues.keyboardShortcuts)
$.facebox(function(){$.get('/javascripts/github.issues.js',function(data){var lines=[],keys=data.replace(/[\s\S]*hotkeys\({([\s\S]+?)}\)[\s\S]*/mg,'$1')
...
this.startingPosition=$.inArray(actor[0],issues)},stopDrag:function(e){var sorting=[]
...
$('#issue_list li').each(function(){sorting.push(this.id.split('_')[1])})
...
if(!done){$('.labels .list').append(label)}},setLabelColors:function(label,hsb,rgb){label.css('backgroundColor',GitHub.Color.rgb2hex(rgb));var borderColor=null
...
return idname.match(/^label(\d+)$/)[1]},saveColor:function(){var id=this.currentId()
var color=this.chooser().find('.colorpicker_hex input').val()
var oldcolor=labels['label'+id]||'#ededed'
oldcolor=oldcolor.replace(/#/g,'')
if(color!=oldcolor){$('div[rel=label'+id+']').css('background-color','#'+color).css('border-color','#'+color)
$('div[rel=label'+id+'] .labeli').css('border-color','#'+color)
...
form.submit()}},rename:function(){var chooser=this.chooser()
...
this.close()}}
GitHub.Color={rgb2hex:function(rgb){return'rgb('+rgb.r+','+rgb.g+','+rgb.b+')'},hsb2hex:function(hsb){return this.rgb2hex(this.hsb2rgb(hsb))},hsb2rgb:function(hsb){var rgb={}
...
return false})
$('.issue.closed').click(function(e){return GitHub.Issues.issueRowClickHandler(e)})
...
return false})
$('#new_issue_comment').submit(function(){if($(this).find('textarea').val().match(/^\W*$/)){var status=$(this).find('.status')
status.text('Comment must not be empty').show()
setTimeout(function(){status.fadeOut('normal')},3000)
return false}else{return true}})
$('.vote').click(function(){var issue_id=$(this).parents('.issue').attr('id').match(/\d+$/)[0],votes=$(this).parents('.voting_box').find('small'),num=parseInt(votes.text())+1,text=num+' vote'
if(num!=1)text+='s'
votes.text(text)
$.post(location.pathname+'/'+issue_id+'/vote',{})
...
chooser.find('.picker').ColorPicker({flat:true,color:labelColor,onChange:function(hsb,hex,rgb){GitHub.Issues.Labels.setLabelColors(chooser.find('.label'),hsb,rgb)}})
chooser.prependTo($(this).parent()).show()}})
$('body').click(function(){if(GitHub.Issues.Labels.Editor.isOpen()){GitHub.Issues.Labels.Editor.saveColor()
GitHub.Issues.Labels.Editor.close()}})
$('.create_label').click(function(){$('#new_label').toggle().find('input[type=text]').focus()
return false})
...
$(this).val('gh-actions')
return $(this).blur()}
var human=$(this).parent().find('option:selected').text(),human=human.slice(2,human.length),updated=[]
$.each(issues,function(i,issue){var div_id='issue_'+issue+'_label_'+label_id
if(human=='Open'){GitHub.Issues.openIssue(issue)}else if(human=='Close'){GitHub.Issues.closeIssue(issue)}else if(human=='Mark as Read'){GitHub.Issues.markAsRead(issue)}else if(human=='Mark as Unread'){GitHub.Issues.markAsUnread(issue)}else if($('#'+div_id).length==0){var label=$('<div id="'+div_id+'" class="label label'+label_id+'"><div class="labeli"><div class="name"><span>'+human+'</span><div class="remove" style="display: none;">x</div></div></div></div>')
$('#issue_'+issue+' .issue_title').before(label)
if(GitHub.Issues.active!=null){$('#issue_'+issue+' .label .remove').show()}
updated.push(issue)}})
if(updated.length>0){$.post($('#new_label').attr('action')+'/'+label_id+'/append',{issues:updated.join(',')})}
$(this).val('gh-actions')
return $(this).blur()})
...
issues_selection_log=[]
return false})
if(GitHub.Issues.collab&&GitHub.Issues.isDraggable()){$('#issues .display .list').sortable({axis:'y',containment:'#issues',handle:'.drag.handle',cancel:'.open, .voting_box',update:renumberCheckboxes,sort:GitHub.Issues.Dragger.sortDrag,start:GitHub.Issues.Dragger.startDrag,stop:GitHub.Issues.Dragger.stopDrag})}
